...
// src/cores-configs/normalConfigs.ts
async function getNormalConfigs(request, env) {
  const { hostName, client, trojanPassword, defaultHttpsPorts, userID } = getParams(request);
  const {
    cleanIPs,
    proxyIP,
    ports,
